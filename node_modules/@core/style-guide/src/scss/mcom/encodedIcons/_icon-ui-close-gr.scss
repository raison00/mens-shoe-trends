@import "../../common/settings/icons_size";

$icon-ui-close-gr: url("data:image/svg+xml;base64,");

.icon-ui-close-gr-full {
	@include svg-icon( $icon-ui-close-gr , $icon-size-full );
}

.icon-ui-close-gr-huge {
	@include svg-icon( $icon-ui-close-gr , $icon-size-huge );
}

.icon-ui-close-gr-large {
	@include svg-icon( $icon-ui-close-gr , $icon-size-large );
}

.icon-ui-close-gr-medium {
	@include svg-icon( $icon-ui-close-gr , $icon-size-medium );
}

.icon-ui-close-gr-small {
	@include svg-icon( $icon-ui-close-gr , $icon-size-small );
}

.icon-ui-close-gr-tiny {
	@include svg-icon( $icon-ui-close-gr , $icon-size-tiny );
}

.icon-ui-close-gr-default {
	@include svg-icon( $icon-ui-close-gr );
}

