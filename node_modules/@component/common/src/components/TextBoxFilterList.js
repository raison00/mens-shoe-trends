...
// an element that implement a list collection (LIs) to be filtered given the user input.
{
  ...
  function self() {
    ...
    const charReplaceRegx = [];
    let countTotal;
    let countShown;
    let countHidden;
    let configRgxpModifiers;
    ...
    // ###Method - countShown
    // Private function that will return the amount of line items that are shown after the filtering is complete.
    // This is accessed via the "getProperty()" function.
    //
    // > parameters
    // >
    // + none
    ...
    // ###Method - countHidden
    // Private function that will return the amount of line items that are hidden after the filtering is complete.
    // This is accessed via the "getProperty()" function.
    //
    // > parameters
    // >
    ...
    // > returns
    // >
    // + *Integer* - Amount of line items that are "hidden" due to not matching the algorithm of filtering.
    // This is accessed via the "getProperty()" function.
    ...
    function replaceSpecChars(str) {
      let retStr = str;
      for (let itr = 0; itr < charReplace.length; itr++) {
        retStr = retStr.replace(charReplaceRegx[itr], charReplace[itr][1]);
      }
      return retStr;
    }
    ...
    function makeIdIfUnknown(str) {
      ...
      if (typeof retStr === 'string') {
        if (retStr.substring(0, 1) !== '#' && retStr.substring(0, 1) !== '.') {
          retStr = `#${retStr}`;
        }
      }
      return retStr;
    ...
    const unbind = function (element, eventType) { // eslint-disable-line no-unused-vars
      const el = makeIdIfUnknown(element);
      if (eventType) {
        el.unbind(eventType);
      } else {
        el.unbind();
    ...
    function showHide(arShow, arHide) {
      ...
      for (itr = 0; itr < shown; itr++) {
        arShow[itr].fadeIn();
      }
      countS = shown;
      for (itr = 0; itr < hidden; itr++) {
        arHide[itr].fadeOut();
      }
      countH = hidden;
    }
    ...
    function analyzeElement(listitem, value, whichone, arShow, arHide) {
      const listItemSel = $(listitem);
      const modifiers = getRgxpModifiers();
      const elementWithText = listItemSel.find(config.textSelector);
      let pattern;

      const srcText = String(elementWithText.length > 0 ? elementWithText.text() : $(listitem).text()).trim();

      const term = strUtil.escapeForRegEx(replaceSpecChars(value));
      pattern = whichone === 'start' ? new RegExp(`^(${term})`, modifiers) : (pattern = new RegExp(`(${term})`, modifiers));
      const classes = config.highlight ? config.highlightClass : undefined;
      ...
      if (replaceSpecChars(srcText).match(pattern)) {
        ...
        arShow[arShow.length] = listItemSel; // eslint-disable-line no-param-reassign
      } else {
        arHide[arHide.length] = listItemSel; // eslint-disable-line no-param-reassign
      }
    }
    ...
    function filterContainsOrStartsWith(value, container, whichone) {
      ...
      if (value === '') {
        ...
        if ((arShow.length === 0 && whichone === 'start' && config.initialNoFilterIfNoMatch) === false) {
          ...
          // STEP: Tricky!!! Under this condition, we actually need to reverse the show/hide arrays, cause we want to show
          // everything and not hide everything.
    ...
    // >
    // + Nothing.
    this.filter = function (elem, cont) {
      eleContainer = cont;
      const element = makeIdIfUnknown(elem);
      const container = makeIdIfUnknown(cont);
    ...
    this.attachTo = function (element, container) {
      ...
      const _this = this;
      const el = makeIdIfUnknown(element);
      const co = makeIdIfUnknown(container);
      $(el).bind('keyup', () => {
        _this.filter(el, co);
      });
    };
