// #MODULE - ResponsiveUI
// > Authors: Dale Gillespie
// >
// > Create Date: Mar 9, 2017
// >
// ##DESCRIPTION: This module handles making page components responsive.

{
  ...
  const mod = {
    ...
    makePageResponsive() {
      // Handle Image Maps
      const $i = $('img.stretch[usemap]');
      if ($i.length > 0) {
        $.each($i, function () {
          $(this).on('mouseover', function () {
            mod.makeImageMapsResponsive($(this));
          });
        });
        $('area').on('mouseover touchstart', (el) => {
          mod.makeImageMapsResponsive($(`img.stretch[usemap=#${el.currentTarget.parentElement.name}]`));
        });
      }
    },

    makeImageMapsResponsive(imageMapCollection) {
      const $imgs = imageMapCollection;
      ...
      const oneImageMap = function () {
        ...
        $imgs.each(function () {
          ...
          $('<img />').load(() => {
            ...
            const wPercent = $img.width() / 100;
            const hPercent = $img.height() / 100;
            const map = $img.attr('usemap').replace('#', '');
            const c = 'coords';
            ...
            $(`map[name="${map}"]`).find('area').each(function () {
              ...
              // STEP: Create an array of the coordinates, and establish another array which will hold the calculated values
              // at the same index locations.
              const coords = $area.data(c).split(',');
              const calc = []; // new Array(coords.length);//[];//[ coords.length ];
              ...
              // why we are using "modulus" 2...we are applying the "width" scaling when its a width value and "height"
              // scaling when its a height value.
          ...
          // STEP: For unit testing and human testing, we are setting a "data-mapped=y" attribute on the image.  This can be used to verify
          // that this was run on a given image.  Could also be used later to select all the images maps that are responsive.
