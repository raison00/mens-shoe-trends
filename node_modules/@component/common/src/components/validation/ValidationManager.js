...
// ##DESCRIPTION: Module to handle multiple validations
{
  ...
  // + *validators* : *Object* | *Array* - list of the validators to run in the form of object: { required: true, minCharacters: 3 }
  // + or list of validators to run in predetermined order in the form of array: [ { required: true }, { minCharacters: 3 } ]
  // + Any method from the Validation module is valid
  //
  // > returns
  // >
  ...
  function validate(fieldName, valueToValidate, validators) {
    ...
    let isValueGood;
    let hasCondition;
    const isValidatorArray = $.isArray(validators);
    ...
    if (validators) {
      if (isValidatorArray) {
        ...
        requiredValidator = $.grep(validators, n => (n.required ? n.required : false));
    ...
    if (isValidatorArray) {
      ...
      $.each(validators, (i, validatorObject) => {
        hasCondition = true;
        // looping each object's prop
        $.each(validatorObject, (validatorName, validatorOptions) => {
          ...
          if (validatorName === 'related') {
            ...
            if (validatorOptions.validators && $.isPlainObject(validatorOptions.validators)) {
              ...
              $.each(validatorOptions.validators, (conditionValidatorName, conditionValidatorOptions) => {
      ...
      // ------------------------ */
      $.each(validators, (validatorName, validatorOptions) => {
        isValueGood = runFieldValidation(valueToValidate, validatorName, validatorOptions);
        if (!isValueGood) {
