...
// if new text is inserted.
{
  ...
  let $this;
  const clearTitle = 'Click to clear this textbox'; // the title we will use for the clear link
  ...
  const clearable = function (elementId, callback) {
    $this = $(`#${elementId}`);
    const inputId = $this.attr('id');
    const containerId = `clearable_${inputId}`;
    const shown = $this.val() !== undefined && $this.val().length > 0 ? '' : 'display:none;';
  ...
  const unbind = function (elementId, eventType) {
    $this = $(`#${elementId}`);
    if (eventType) {
