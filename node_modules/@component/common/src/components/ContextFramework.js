...
// }
{
  ...
  const Store = require('../util/ClientSideStorage');
  ...
  // >
  // + Nothing.
  const setPostalCode = function (val) {
  ...
  function hasLocationObjectInStorage() {
    const a = Store.getSession(constants.KEY_LOCATION);
  ...
  function getLocationObject() {
    return hasLocationObjectInStorage()
      ? JSON.parse(Store.getSession(constants.KEY_LOCATION))
      : {
        preferred: '',
        locations: {},
      };
  ...
  const getPostalCodePreferred = function () {
  ...
  // >
  // + Nothing.
  const setPostalCodePreferred = function (val) {
  ...
  function getCityPreferred() {
    ...
    let retval = '';
    if (hasLocation(loc.preferred) && loc.locations[loc.preferred].address) {
      const a = loc.locations[loc.preferred].address;
  ...
  function getLatLng() {
    ...
    if (Cookie.get(constants.latlngDefault, constants.cookieSelectedDictionary)) {
      const v = Cookie.get(constants.latlngDefault, constants.cookieSelectedDictionary).split(',');
      if (v.length >= 2) {
        lat = v[0];
        lng = v[1];
      }
  ...
  function setLatLng(val) {
    Cookie.setExpires(new Date(new Date().getTime() + constants.dateAdd));
    const v = String(`${val.lat},${val.lng}`);
  ...
  function setPreferred(val) {
    ...
    /*
        ...
        */
    loc = getLocation(val);
    Promise.all([import('./validation/Validation')]).then(([Valid]) => {
  ...
  // + device
  function getDevice() {
    const device = Cookie.get('DT', 'MISCGCs');
    return device;
  }
  ...
  // + locale
  function getLocale() {
    const locale = Cookie.get('shippingCountry');
    return locale;
  }
  ...
  module.exports = {
    ...
    user: {
      getPostalCodePreferred,
      setPostalCodePreferred,
